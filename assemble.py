...
class Assembler():
    ...
    def parse(self, fname):
        with mrc.open(fname, 'r', permissive=True) as f:
            try:
                self._stack_data = f.data[:,::self.step,::self.step]
            except IndexError:
                self._stack_data = f.data
            self._h = f.header
            self._eh = np.frombuffer(f.extended_header, dtype='i2')

    def assemble(self):
        dimensions = self._stack_data.shape
        ...
        if len(dimensions) == 3:
            ...
            for i in range(dimensions[0]):
                ...
                np.add.at(self.data, (cx+pos_x[i], cy+pos_y[i]), self._stack_data[i])
            sys.stderr.write('\n')
